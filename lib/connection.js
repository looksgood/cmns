...
var Connection = function(options) {
  ...
  this.options = {
    ...
    keyData: null,
    passphrase: null,
    gateway: "gateway.push.yunos.com",
    port: 2195,
    enhanced: true,
    errorCallback: null,
    cacheLength: 1000,
...
Connection.prototype.sendNotification = function(notification) {
  ...
  this.openSocket().then(function() {
    ...
    if (this.options.enhanced) {
      ...
      // Identifier
      data.writeUInt32BE(notification._uid, pos);
      pos += 4;

      // Expiry
      data.writeUInt32BE(notification.expiry, pos);
      pos += 4;
    } else {
    ...
    // Token Length
    data.writeUInt16BE(token.length, pos);
    pos += 2;
    // Device Token
    pos += token.copy(data, pos, 0);
    // Payload Length
    data.writeUInt16BE(messageLength, pos);
    pos += 2;
    //Payload
